...
public:
	...
	FORCEINLINE void HealHp(const float InHealAmount) { CurrentHp = FMath::Clamp(CurrentHp + InHealAmount, 0, GetTotalStat().MaxHp); OnHpChanged.Broadcast(CurrentHp); }
...
protected:
	...
	UPROPERTY(Transient, VisibleInstanceOnly, Category = Stat) // CurrentHp는 바뀌므로 Transient 지정해서 디스크 저장X
	float CurrentHp;
