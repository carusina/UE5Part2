...
void AABStageGimmick::OnGateTriggerBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
    ...
    FName ComponentTag = OverlappedComponent->ComponentTags[0]; // GateTrigger에서 ComponentTag 가져오기
    FName SocketName = FName(*ComponentTag.ToString().Left(2)); // +X, -X, +Y, -Y
    check(Stage->DoesSocketExist(SocketName));                  // Stage에 해당 Socket이 있는지 확인

    FVector NewLocation = Stage->GetSocketLocation(SocketName); // Socket의 위치 가져오기
    TArray<FOverlapResult> OverlapResults;
    
    // 자신을 제외한 모든 StaticObject가 Sphere(775.0f)안에 있는지 검사
    FCollisionQueryParams CollisionQueryParam(SCENE_QUERY_STAT(GateTrigger), false, this);
    bool bResult = GetWorld()->OverlapMultiByObjectType(OverlapResults,
                                                        NewLocation,
                                                        FQuat::Identity,
                                                        FCollisionObjectQueryParams::InitType::AllStaticObjects,
                                                        FCollisionShape::MakeSphere(775.0f),
                                                        CollisionQueryParam);
    if (!bResult) // 없다면 새로운 Stage 생성
    {
...
void AABStageGimmick::OnOpponentSpawn()
{
    ...
    AABCharacterNonPlayer* ABOpponentCharacter = GetWorld()->SpawnActorDeferred<AABCharacterNonPlayer>(OpponentClass, SpawnTransform); // NPC 지연 생성
    if (ABOpponentCharacter)
    {
        ABOpponentCharacter->OnDestroyed.AddDynamic(this, &AABStageGimmick::OnOpponentDestroyed);
        ABOpponentCharacter->SetLevel(CurrentStageNum);
        ABOpponentCharacter->FinishSpawning(SpawnTransform); // FinishSpawning 호출 이후에 ABOpponentCharacter의 BeginPlay 시작
    }
}
...
void AABStageGimmick::OnRewardTriggerBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
    ...
    {
        if (RewardBox.IsValid()) // TWeakObjectPtr이기 때문에 유효한지 확인
        {
