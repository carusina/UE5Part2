...
void UABHUDWidget::NativeConstruct()
{
	...
	HpBar = Cast<UABHpBarWidget>(GetWidgetFromName(TEXT("WidgetHpBar"))); // WBP_ABHUD와 일치해야함
	ensure(HpBar);

	CharacterStat = Cast<UABCharacterStatWidget>(GetWidgetFromName(TEXT("WidgetCharacterStat")));
	ensure(HpBar);

	IABCharacterHUDInterface* HUBPawn = Cast<IABCharacterHUDInterface>(GetOwningPlayerPawn());
	if (HUBPawn)
	{
		HUBPawn->SetupHUDWidget(this);
	}
}
